...
if __name__ == "__main__":
    ...
    similiarities: dict[str, list[tuple[float, str]]] = {}

    for post in all_posts:
        similiarities[post.title] = []

    for post_1, post_2 in itertools.combinations(all_posts, 2):
        similiarities[post_1.title].append((post_1.distance_to(post_2), post_2.title))

    for post in all_posts:
        similiarities[post.title].sort(key=lambda entry: -entry[0])
    ...
    for post in all_posts:
        ...
        for related in similiarities[post.title][:1]:
            relations_graph.edge(post.title, related[1], color="white")
